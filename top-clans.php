...
$num_elements = 30;
$total1 = mysql_result(mysql_query("SELECT COUNT(*) FROM `clan_data` WHERE `clan_exp` > 0"),0,0); 
$num_pages = ceil($total1 / $num_elements);
if ($p > $num_pages) $p = $num_pages;
$start = ($p - 1) * $num_elements; 
$sel = "SELECT * FROM `clan_data` WHERE `clan_exp` > 0 ORDER by `clan_exp` DESC LIMIT ".$start.", ".$num_elements;
			...
			<nav>
				<ul>
					...
					<li class="dep01-5">
						...
						<div class="dep02"></div>
						<ul>
							<li><a href=""><h3>ОСОБЕННОСТЬ</h3></a></li>
							<li><a href=""><h3>РЕЙТЫ</h3></a></li>
						</ul>
					</li>
				</ul>
			</nav>
        ...
        <div class="sub-content detail-content schedule-content">
            ...
            <div class="schedule-tbl">
                <table>
                    ...
                    <tbody>
					 <tr>
                                <td colspan="6">Данный ТОП Кланов по боевому опыту клана.</td>
								<br>
								<td colspan="6">Обновление: каждый вторник недели!</td>
								<div id="wrap"> 
	...
	<table class="simple-little-table" cellspacing='0'>
		...
		<tr>
		   <!-- <center><img src="1.jpg"></center>
			...
			<th width="25%" height="16" class="s_top"><b>Название</b></th>
			<th height="16" class="s_top" align="center"><b>Уровень</b></th>
			<th width="10%" height="16" class="s_top" align="center"><b>ГП</b></th>
			<th width="10%" height="16" class="s_top" align="center"><b>Боевой опыт</b></th>
		</tr>

<?php
$name_array = array('Новички','Новички','Ефрейторы','Мл.сержанты','Сержанты','Ст.сержанты','Старшины','Прапорщикы','Ст.Прапорщикы','Мл.Лейтенанты','Лейтенанты','Ст.Лейтенант','Капитан','Майор','Подполковник','Полковник','Генерал-Майор','Генерал-Лейтенант','Генерал-Полковник','Генерал-Армии','Маршал');
if(mysql_num_rows($result)>0)
{
	...
	{ 
			...
			echo '<td height="16" align="center" class="s_1"><div style="float:left;">&nbsp&nbsp'.$row['clan_name'].'</div></td>';
			echo '<td height="16" class="s_1"><div style="float:left;">'.$name_array[$row['clan_rank']].'</div></td>';
			echo '<td height="16" align="center">'.$row['pontos'].'</td>';
			echo '<td height="16" align="center">'.$row['clan_exp'].'</td>';
			echo '</tr>';
	}
}
?>
